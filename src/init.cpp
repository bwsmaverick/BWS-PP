...
void Shutdown()
{
    ...
    RenameThread("bitcoinwspectrum-shutoff");
...
std::string HelpMessage(HelpMessageMode hmm)
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: bitcoinwspectrum.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: bitcoinwspectrumd.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 7201 or testnet: 7203)") + "\n" +
...
{
    RenameThread("bitcoinwspectrum-loadblk");
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    //
    // bwscoin: -mintxfee and -minrelaytxfee options of bitcoin disabled
    ...
    if (mapArgs.count("-checkpointkey")) // bwscoin: checkpoint master priv key
    {
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Bitcoinwspectrum is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("Bitcoinwspectrum version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "Bitcoinwspectrum server starting\n");
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of Bitcoinwspectrum") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart Bitcoinwspectrum to complete") << "\n";
