...
// bwscoin: remove transaction from mempool
...
// bwscoin: reserve balance from being staked for network protection
...
// bwscoin: check wallet integrity
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop Bitcoinwspectrum server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Bitcoinwspectrum server stopping";
}
...
void StartRPCThreads()
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use bitcoinwspectrumd";
