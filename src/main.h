...
static const int64 MAX_MINT_PROOF_OF_WORK = 110000000 * COIN;
static const int64 MIN_TXOUT_AMOUNT = MIN_TX_FEE;
/** Coinbase transaction outputs can only be spent after this number of new blocks (network rule) */
static const int COINBASE_MATURITY_BWS = 500;
/** Threshold for nLockTime: below this value it is interpreted as block number, otherwise as UNIX timestamp. */
static const int STAKE_TARGET_SPACING = 2 * 60; // 2-minute block spacing 
static const int STAKE_MIN_AGE = 60 * 60 * 6; // minimum age for coin age
...
static const uint256 hashGenesisBlockOfficial("0x000002d5125cec30110fa355039b9059914a9a24f76535ce82ea68ae7ea223a4");
static const uint256 hashGenesisBlockTestNet("0x00000001da9a49dab79c5ed11cecb19a1ba79959789509ad0d15249d720c72a5");
...
public:
    ...
    {
        // bwscoin: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CValidationState &state, CCoinsViewCache &view, uint64& nCoinAge) const;  // bwscoin: get transaction coin age
...
public:
    ...
    bool fCoinStake;      // bwscoin: if the outpoint was the last unspent: whether it belonged to a coinstake
    unsigned int nTime;   // bwscoin: if the outpoint was the last unspent: its tx timestamp
...
public:
    ...
    // bwscoin: whether transaction is a coinstake
    bool fCoinStake;

    // bwscoin: transaction timestamp
    unsigned int nTime;
    ...
    bool IsCoinStake() const { // bwscoin: coinstake
        return fCoinStake;
    }
    ...
    bool Spend(const COutPoint &out, CTxInUndo &undo) {
        ...
        if (vout.size() == 0) {
            ...
            undo.fCoinStake = fCoinStake;  // bwscoin
            undo.nTime = nTime;            // bwscoin
        }
        return true;
    }
...
public:
    ...
    // bwscoin: block signature - signed by coin base txout[0]'s owner
    ...
    // bwscoin: two types of block: proof-of-work or proof-of-stake
    ...
    // bwscoin: get max transaction timestamp
    ...
    bool GetCoinAge(uint64& nCoinAge) const; // bwscoin: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // bwscoin: entropy bit for stake modifier if chosen by modifier
};
...
public:
    ...
    // (memory only) Total amount of trust score (bwscoin proof-of-stake difficulty) in the chain up to and including this block
    uint256 nChainTrust;
    ...
    // bwscoin: money supply related block index fields
    int64 nMint;
    int64 nMoneySupply;

    // bwscoin: proof-of-stake related block index fields
    unsigned int nFlags;  // bwscoin: block index flags
    enum
    {
